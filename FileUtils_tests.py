...
def test_getFileContent():
    desc = "read the content"
    expected = "Sir Charles Lyell is best known as the author of Principles of Geology."
...
def test_appendToFile():
    desc = "Appending to a file"
    expected = "Thank you"
    script_folder = os.path.dirname(os.path.abspath(__file__))
    fileName = os.path.join(script_folder,"appendToFileTest.txt")
    FU.writeToFile(fileName,"")
    FU.appendToFile(fileName, "Thank you")
    actual = FU.getFileContent(fileName)
    print_test_results(FU.appendToFile, desc, expected, actual)
